// Programación Funcional y ES6

/**
 * Ejercicio 1
  const arr = [1, 2, 3, 4, 5, 6];

  A.- utilizar el método map de los arreglos para duplicar cada elemento
      [2, 4, 6, 8, 10, 12]

  B.- utilizar el método filter para filtrar los números pares de arr
      [2, 4, 6]

   C.- utilizar el método reduce para sumar todos los elementos
      21
 */
   ...
   console.log("EJERCICIO 1");
    resultado = arr.map(function(val){return val*2})
    console.log(`item A : ${resultado}`);

    resultado = arr.filter(function(val){return val%2 === 0;})
    console.log(`item B : ${resultado}`)

    resultado = arr.reduce(function(suma,val){return suma+val},0);
    console.log(`item C : ${resultado}`)


/**
 * Ejercicio 2
    For this exercise use the filter , map, y reduce methods to generate some reports del Array Cars[]

    A.- Create a function ferraris that returns all the cars of the Ferrari brand.

    B.- Create a function nineties that returns all the cars with models between 1980 and 1990.

    C.- Create a function list that returns an array of strings with the same number of cars. Each element should contain the string “This from costs $” (e.g. for the first element it should say “This Mazda from 1989 costs $5241”).

    D.- Create a function bmwSum that returns the sum of all prices of the BMW cars.
...
function ferraris(cars){
    ...
    return cars.filter(function(car){return car.brand==='Ferrari'});        
}

function nineties(cars){
    return cars.filter(function(car){return car.model>=1980 && car.model<=1990},);
}

function list(cars){
    return cars.map(function(car){ return `This ${car.brand} from ${car.model} costs $ ${car.price}`});
}

function bmwSum(cars){
    return cars.reduce(function(totalPrices,car){if (car.brand=='BMW'){totalPrices =totalPrices+car.price} return totalPrices},0);
}

console.log("Resultado de Ejercicio 2");
console.log("Pregunta A.- ");
console.log(ferraris(cars));
console.log("Pregunta B.- ");
console.log(nineties(cars));
console.log("Pregunta C.- ");
console.log(list(cars));
console.log("Pregunta D.- ");
